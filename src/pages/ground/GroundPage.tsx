...
import { useBalances } from "../../hooks/ground/clients/useBalances";
import useBalancesBalance from "../../hooks/ground/clients/useBalancesBalance";
...
const GroundPage = () => {
  ...
  const { data: balance } = useBalancesBalance();
                  ...
                  >
                    ...
                    <div className="p-3 flex-grow-1  fs-4 align-self-start">
                      {(sell?.destine?.name
                        ...
                        sell.product?.name ||
                        (sell.client?.name
                          ? `Entrou de ${sell.client?.name}`
                          : "") ||
                        (sell.from_destine?.name
                          ? `Reembolso de ${sell.from_destine?.name}`
                          : "")}
                    </div>
                    <SellValue
                      sell={sell}
                      remain={handleSetRemain(
                        sell.destine ? -sell.value : sell.value
                      )}
