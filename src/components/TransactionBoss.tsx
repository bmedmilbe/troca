...
const TransactionBoss = ({ deliver, boss }: Props) => {
  const query_param = {
    boss: undefined,
    is_charge: undefined,
    deliver: undefined,
  ...
  const { data: balance } = useRemain({ boss: boss?.id, deliver: deliver?.id });
  // console.log(balance);
                  ...
                  {/* <TransactionValue
                    ...
                    remain={handleSetRemain(
                      transaction.completed
                        ? handleSetRemain(values.remain - ((transaction.value || 0) || 0))
                        : values.remain
                    )}
                  /> */}
...
export default TransactionBoss;
